...
;(function($){$.fn.camera = function(opts, callback) {
	...
	var defaults = {
		...
		overlayer			: true,	//a layer on the images to prevent the users grab them simply by clicking the right button of their mouse (.camera-overlayer)
...
////////callbacks
	...
	var wrap = $(this).addClass('camera-wrap');
	
	wrap.wrapInner(
        '<div class="camera-src" />'
		).wrapInner(
	    '<div class="camera-fakehover" />'
		);
		
	var fakeHover = $('.camera-fakehover',wrap);
	...
	if(opts.overlayer == true){
		fakeHover.append(
			'<div class="camera-overlayer"></div>'
			)
	}
		fakeHover.append(
        '<div class="camera-target-content"></div>'
		);
	...
	if(loader == 'pie'){
		...
		fakeHover.append(
			'<div class="camera-bar" style="display:none"></div>'
			)
	}
	
	if(opts.playPause==true){
		fakeHover.append(
        '<div class="camera-commands"></div>'
		)
	}
		
	if(opts.navigation==true){
		fakeHover.append(
			'<div class="camera-prev"><span></span></div>'
			).append(
			'<div class="camera-next"><span></span></div>'
			);
	}
		
	if(opts.thumbnails==true){
		wrap.append(
			'<div class="camera-thumbs-cont" />'
			);
	}
	
	if(opts.thumbnails==true && opts.pagination!=true){
		$('.camera-thumbs-cont',wrap).wrap(
			'<div />'
			).wrap(
				...
				'<div class="camera-command-wrap" />'
			);
	}
		
	if(opts.pagination==true){
		wrap.append(
			'<div class="camera-pag"></div>'
			);
	}
	...
	for (loopMove=0;loopMove<amountSlide;loopMove++)
	{
		...
		if(allLinks[loopMove]!=''){
			...
			//
			$('.camera-target-content .cameraContent:eq('+loopMove+')',wrap).append('<a class="camera-link" href="'+allLinks[loopMove]+'" '+dataBox+' target="'+allTargets[loopMove]+'"></a>');
		}
	...
	{
		cameraCont.append('<div class="cameraSlide cameraSlide-'+loop+'" />');
		var div = $('> div:eq('+loop+')',elem);
		target.find('.cameraSlide-'+loop).clone(div);
	}
	
	
	function thumbnailVisible() {
		var wTh = $(thumbs).width();
		$('li', thumbs).removeClass('camera-visThumb');
		$('li', thumbs).each(function(){
			var pos = $(this).position(),
				...
				ulLeft = offDiv-offUl;
				if(ulLeft>0){
					$('.camera-prevThumbs',camera_thumbs_wrap).removeClass('hideNav');
				} else {
					$('.camera-prevThumbs',camera_thumbs_wrap).addClass('hideNav');
				}
				if((ulW-ulLeft)>wTh){
					$('.camera-nextThumbs',camera_thumbs_wrap).removeClass('hideNav');
				} else {
					$('.camera-nextThumbs',camera_thumbs_wrap).addClass('hideNav');
				}
				var left = pos.left,
					right = pos.left+($(this).width());
				if(right-ulLeft<=wTh && left-ulLeft>=0){
					$(this).addClass('camera-visThumb');
				}
		});
	}
	...
	if(elem.length!=0){
		...
		$('iframe',fakeHover).each(function(){
			...
			$('.camera-target-content .cameraContent:eq('+divInd+')',wrap).append(t);
		});
		function imgFake() {
				$('iframe',fakeHover).each(function(){
		...
		if($(pagination).length) {
			$(pagination).append('<ul class="camera-pag-ul" />');
			var li;
			for (li = 0; li < amountSlide; li++){
				$('.camera-pag-ul',wrap).append('<li class="pag-nav-'+li+'" style="position:relative; z-index:1002"><span><span>'+li+'</span></span></li>');
			}
			$('.camera-pag-ul li',wrap).hover(function(){
		...
		if($(thumbs).length) {
			...
			if(!$(pagination).length) {
				$(thumbs).append('<div />');
				$(thumbs).before('<div class="camera-prevThumbs hideNav"><div></div></div>').before('<div class="camera-nextThumbs hideNav"><div></div></div>');
				...
				$.each(allThumbs, function(i, val) {
					if($('> div', elem).eq(i).attr('data-thumb')!='') {
						var thumbUrl = $('> div', elem).eq(i).attr('data-thumb'),
							newImg = new Image();
						newImg.src = thumbUrl;
						$('li.pag-nav-'+i,pagination).append($(newImg).attr('class','camera-thumb').css({'position':'absolute'}).animate({opacity:0},0));
						$('li.pag-nav-'+i+' > img',pagination).after('<div class="thumb-arrow" />');
						$('li.pag-nav-'+i+' > .thumb-arrow',pagination).animate({opacity:0},0);
					}
				});
			}
		}
	...
	function nextSlide(navSlide){ 
		...
		$('.camerarelative',slide).append($('> div ',elem).eq(slideI).find('> div.camera-effected'));

		$('.camera-target-content .cameraContent:eq('+slideI+')',wrap).append($('> div ',elem).eq(slideI).find('> div'));
		...
		if(!$('.imgLoaded',slide).length){
			...
			if (!$(imgLoaded).get(0).complete || wT == '0' || hT == '0' || typeof wT === 'undefined' || wT === false || typeof hT === 'undefined' || hT === false) {
				...
				imgLoaded.onload = function() {
					...
					target.find('.cameraSlide-'+slideI).hide().css('visibility','visible');
					resizeImage();
					nextSlide(slideI+1);
				};
			}
			...
			$.each(arr, function(index, value) {
				...
				if($(pagination).length){
					$('.camera-pag li',wrap).removeClass('cameracurrent');
					$('.camera-pag li',wrap).eq(slideI).addClass('cameracurrent');
				}
				...
				if($(pagination).length){
					$('.camera-pag li',wrap).click(function(){
						if(!elem.hasClass('camerasliding')){
				...
				if($(thumbs).length) {
					...
					$('.pix-thumb img',thumbs).click(function(){
						if(!elem.hasClass('camerasliding')){
							...
							if(idNum!=curNum) {
								...
								imgFake();
								$('#'+pieID+', .camera-canvas-wrap',camera_thumbs_wrap).animate({opacity:0},0);
								$('.pix-thumb',thumbs).removeClass('cameracurrent');
					...
					$('.camera-thumbs-cont .camera-prevThumbs',camera_thumbs_wrap).hover(function(){
						$(this).stop(true,false).animate({opacity:1},250);
					},function(){
						$(this).stop(true,false).animate({opacity:.7},250);
					});
					$('.camera-prevThumbs',camera_thumbs_wrap).click(function(){
						var sum = 0,
							wTh = $(thumbs).outerWidth(),
							offUl = $('ul', thumbs).offset().left,
							offDiv = $('> div', thumbs).offset().left,
							ulLeft = offDiv-offUl;
							$('.camera-visThumb',thumbs).each(function(){
					...
					$('.camera-thumbs-cont .camera-nextThumbs',camera_thumbs_wrap).hover(function(){
						$(this).stop(true,false).animate({opacity:1},250);
					},function(){
						$(this).stop(true,false).animate({opacity:.7},250);
					});
					$('.camera-nextThumbs',camera_thumbs_wrap).click(function(){
						var sum = 0,
							wTh = $(thumbs).outerWidth(),
							ulW = $('ul', thumbs).outerWidth(),
							offUl = $('ul', thumbs).offset().left,
