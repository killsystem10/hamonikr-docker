...
//			$source .= '<a href="javascript:;" target="_blank">'.$val.'</a>&nbsp;&nbsp;&nbsp;';
	...
	<tr>
		...
		<td colspan="4" style="">
			...
			<a href=JavaScript:newPopup("http://git.hamonikr.org:3000/redmine/projects/'.$k_P.'-test/repository/revisions/master/entry/'.$arSRCpos[0].'#L'.$arSRCpos[1].'")>소스보기</a> </div>
		</td>
	</tr>
		';
...
$tot = $DB->FRow($sql);
$sql = "select count(h.ano) from htrpass1 as h where h.project='".$k_P."' and (h.msgstr<>'' or h.msgcommit is not null) and h.msgid<>''".$Twhere;
$ing = $DB->FRow($sql);
