...
## 전달값
define(_L, '_PB='._PB.'&_PS='._PS.'&c='.$k_c.'&_N='._N.'&Colm='.$k_Colm.'&Query='.$k_Query.'&P='.$k_P.'&Type='.$k_Type);
...
if($k_Type!='') $Twhere.=" and h.msgcommit is null";

$sql = "select h.ano from htrpass1 as h where 1=1 and h.msgid<>'' and h.msgstr<>'' and h.project='".$k_P."'".$where.$Twhere."";
$r1 = $DB->NRow($sql);
$cnt = $r1;
$Number=($cnt - (_N - 1) * _PS);
$sql = "
...
where
	h.project='".$k_P."'".$where.$Twhere." and h.msgid<>'' and h.msgstr<>''
group by
	h.ano
order by
...
$tot = $DB->FRow($sql);
$sql = "select count(h.ano) from htrpass1 as h where h.project='".$k_P."' and h.msgstr<>'' and h.msgid<>''".$Twhere;
$ing = $DB->FRow($sql);
