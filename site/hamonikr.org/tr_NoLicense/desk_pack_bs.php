...
$sql = "select h.ano from htrpass3 as h where 1=1 and h.msgid<>'' and h.project='".$k_P."'".$where.$Twhere."";
$r1 = $DB->NRow($sql);
$cnt = $r1;
$Number=($cnt - (_N - 1) * _PS);
$sql = "
...
left join
	transfer as t
on
	h.ano=t.ano and h.project=t.project and t.status='y' and t.page='D'
where
...
if($res['Count'] > 0)
{
	...
	{
		$source = $S = $S1 = $S2 = '';
		$S2 = str_replace(chr(10), ' ', $res[$i]['filename']);
		$S1 = explode(' ', $S2);
...
//			$source .= '<a href="javascript:;" target="_blank">'.$val.'</a>&nbsp;&nbsp;&nbsp;';
	...
	<tr align="center" style="border-top:2px solid #666666;">
		...
		<td align="left" style="line-height:20px;">
			...
			<div style=""><ul style="border:0px solid;float:left;height:100%;">번 역 :&nbsp;</ul><ul class="bold F_00" style="float:left;width:90%;border:0px solid;word-break:break-all;color:#ff6600;height:100%;">'.RE(SLASH(($res[$i]['msgcommit']!=null)?$res[$i]['msgcommit']:$res[$i]['msgstr'],1),$k_Query,1).'</ul></div>
		</td>
		<td align="right" style="line-height:20px;"><span class="F_00">'.strlen($res[$i]['msgid']).' Bytes</span><br /><span class="F_00">'.strlen(SLASH(($res[$i]['msgcommit']!=null)?$res[$i]['msgcommit']:$res[$i]['msgstr'],1)).' Bytes</span></td>
...
<input type="hidden" name="Kdiv" value="desk_write" />
...
# 총 참여자
$sql = "select count(distinct(writer)) from transfer where project='".$k_P."' and page='D'";
$man = $DB->FRow($sql);

#진척율
$sql = "select count(h.ano) from htrpass3 as h where h.msgid<>'' and h.project='".$k_P."'".$Twhere;
$tot = $DB->FRow($sql);
$sql = "select count(h.ano) from htrpass3 as h where h.project='".$k_P."' and (h.msgstr<>'' or h.msgcommit is not null) and h.msgid<>''".$Twhere;
$ing = $DB->FRow($sql);

$sql = "select count(distinct(project)) as cntP, count(ano) as cntA from htrpass3 where left(wdate,10)='".date("Y-m-d")."' and project='".$k_P."'";
$tod = $DB->FRow($sql);
	...
	<tr height="28">
		<td align="left">
			<span class="btn-lg-3" style="float:left;padding-right:10px;">
				<a class="btn btn-info" href="desk_pack_bs.php?P='.$k_P.'">전체 리스트</a>
				<a class="btn btn-warning" href="desk_pack_bs.php?Type=no&P='.$k_P.'">미번역</a>
			</span>
			<span class="btn-lg-3" style="float:left;padding-right:10px;">
...
function CONFIRM_D(T){if(confirm("선택하신 번역내용을 최종 번역으로 승인하시겠습니까?")){document.Cform.T.value=T;document.Cform.submit();}}
function VOTE(T){if(confirm("이 번역내용을 좋은 번역으로 추천합니다.\n\n한번 추천하신 번역내용은 더 이상 추천할 수 없습니다.")){document.Cform.Kdiv.value="desk_vote";document.Cform.T.value=T;document.Cform.submit();}}
function WRT(AA, PP){var xetime = new Date();if($("#DV_"+AA).css("display")=='none') {var URL = "desk_data<?=_LINK_;?>.php?A="+AA+"&P="+PP+"&time="+(Math.random()*xetime.getTime());var xmlhttp = null;if(window.XMLHttpRequest) {xmlhttp = new XMLHttpRequest();} else {xmlhttp = new ActiveXObject("Microsoft.XMLHTTP");}xmlhttp.open('GET', URL, true);xmlhttp.onreadystatechange = function() {if(xmlhttp.readyState==4 && xmlhttp.status == 200 && xmlhttp.statusText=='OK') {var ret = xmlhttp.responseText;$("#DV_"+AA).html(ret);$("#DV_"+AA).show(700);}}
xmlhttp.send('');
...
function Inst(A, P){if(DFAlert("msg_"+A, "Iform_"+A)==false) return;var Fdata = "A="+A+"&P="+P+"&C="+$("#msg_"+A).val()+"&Kdiv=desk_write";
$.ajax({url : "proc.php",type : "post",	data : Fdata,success : function(data){alert("등록되었습니다");var URL = "desk_data<?=_LINK_;?>.php?A="+A+"&P="+P;var xmlhttp = null;if(window.XMLHttpRequest) {xmlhttp = new XMLHttpRequest();} else {xmlhttp = new ActiveXObject("Microsoft.XMLHTTP");}xmlhttp.open('GET', URL, true);xmlhttp.onreadystatechange = function() {if(xmlhttp.readyState==4 && xmlhttp.status == 200 && xmlhttp.statusText=='OK') {var ret = xmlhttp.responseText;$("#DV_"+A).html(ret);$("#DV_"+A).show(100);}$("#msg_"+A).val("");}
xmlhttp.send('');
},error : function(){alert("등록중 오류가 발생하였습니다.\n\n다시 시도해 주세요");}});}
<?
/*function WRT(AA, PP){if($("#DV_"+AA).css("display")=='none') {$(".DVVIEW").hide();var URL = "desk_data<?=_LINK_;?>.php?A="+AA+"&P="+PP;var xmlhttp = null;if(window.XMLHttpRequest) {xmlhttp = new XMLHttpRequest();} else {xmlhttp = new ActiveXObject("Microsoft.XMLHTTP");}xmlhttp.open('GET', URL, true);xmlhttp.onreadystatechange = function() {if(xmlhttp.readyState==4 && xmlhttp.status == 200 && xmlhttp.statusText=='OK') {var ret = xmlhttp.responseText;$("#DV_"+AA).html(ret);$("#DV_"+AA).show(700);}}
xmlhttp.send('');
...
<div id="wrap" class="bs3-wrap" style="padding-top:30px">
	<div id="left_menu">
		<ul id="menu">
				...
				<ul>
					<p>
					<span style="padding-bottom:5px;"><a href="korean<?=_LINK_;?>.php" class="btn btn-default">패키지별 한글화</a></span>
