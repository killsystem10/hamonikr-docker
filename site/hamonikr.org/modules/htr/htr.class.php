 ...
 * @brief  htr module high class
 **/

class htr extends ModuleObject
{
	...
	function moduleInstall()
	{
		...
		$oModuleController->insertTrigger('member.getMemberMenu', 'htr', 'controller', 'triggerMemberMenu', 'after');
	...
	function checkUpdate()
	{
		...
		if(!$oModuleModel->getTrigger('member.getMemberMenu', 'htr', 'controller', 'triggerMemberMenu', 'after')) return true;

		// 2011. 09. 20 when add new menu in sitemap, custom menu add
		if(!$oModuleModel->getTrigger('menu.getModuleListInSitemap', 'htr', 'model', 'triggerModuleListInSitemap', 'after')) return true;
		return false;
	}
	...
	function moduleUpdate()
	{
		...
		if(!$oModuleModel->getTrigger('member.getMemberMenu', 'htr', 'controller', 'triggerMemberMenu', 'after'))
		{
			$oModuleController->insertTrigger('member.getMemberMenu', 'htr', 'controller', 'triggerMemberMenu', 'after');
		}

		// 2011. 09. 20 when add new menu in sitemap, custom menu add
		if(!$oModuleModel->getTrigger('menu.getModuleListInSitemap', 'htr', 'model', 'triggerModuleListInSitemap', 'after'))
		{
			$oModuleController->insertTrigger('menu.getModuleListInSitemap', 'htr', 'model', 'triggerModuleListInSitemap', 'after');
		}
	...
	{
		$output = executeQueryArray("htr.getAllHtr");
		if(!$output->data) return new Object();
		@set_time_limit(0);

		$oModuleController = getController('module');

		foreach($output->data as $htr)
		{
			$oModuleController->deleteModule($htr->module_srl);
		}
