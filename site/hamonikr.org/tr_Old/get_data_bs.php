...
on
	t.t_idx=v.t_idx
where
	a.project='".$k_P."' and t.project=a.project and t.ano=a.ano and a.ano='".$k_A."'
order by
...
//if($res[$i]['uid']!=$member['user_id']){
	...
	if($_SESSION['group']=='writer'||$_SESSION['group']=='all' && $res[$i]['writer']!=$member['user_id']) {
		echo '<a class="btn-success btn btn-xs" style="vertical-align:middle;" onclick="javascript:VOTE(\''.$res[$i]['t_idx'].'\');">추천</a>'.$res[$i]['uid'];}
...
//	}
	...
	elseif($res[$i]['writer']==$member['user_id']) {
		...
		if($res[$i]['status']=='n' && $res[$i]['vote']=='0'){
			//echo '<a href="javascript:;" class="btn btn-info btn-xs">수정</a><a href="javascript:Del(\''.$res[$i]['t_idx'].'\');" class="btn btn-danger btn-xs">삭제</a>';
...
<?
	if($_SESSION['group']=='commiter' || $_SESSION['group']=='all') 
		echo '		<td><a href="javascript:;" onclick="javascript:CONFIRM(\''.$res[$i]['t_idx'].'\');" class="btn btn-danger btn-xs">Commit</a></td>'.chr(10);
?>
	</tr>
<?
	}
...
?>
	...
	<tr bgcolor="#ffffff" height="27" align="center" style="font-size:12px;font-family:'Malgun Gothic', Verdana;">
		...
		<td>0</td>
		<td></td>
<?if($_SESSION['commiter']=='0') echo '		<td></td>'.chr(10);?>
	</tr>
