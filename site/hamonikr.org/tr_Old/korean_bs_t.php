...
## 전달값
define(_L, '_PB='._PB.'&_PS='._PS.'&c='.$k_c.'&_N='._N.'&Colm='.$k_Colm.'&Query='.$k_Query.'&Type='.$k_Type);
## 정렬
...
elseif($k_Query!='' && $k_Colm=='') $where = " and (h.project like '%".$k_Query."%' or h.msgid like '%".$k_Query."%' or h.msgstr like '%".$k_Query."%')";
if($k_Type!='') $Twhere=" and (h.msgstr='' and h.msgcommit is null)";

$sql = "select count(h.ano) from htrpass1 as h where 1=1".$where.$Twhere." group by h.project";
$r1 = $DB->NRow($sql);
$cnt = $r1;
$Number=($cnt - (_N - 1) * _PS);
...
left join
	htrpass1 as h1
on
	h.ano=h1.ano and h.project=h1.project and h.msgid<>''
left join
	htrpass1 as h2
on
	h.project=h2.project and h.ano=h2.ano and h.msgid<>'' and h.msgstr<>''
left join
	transfer as t
on
	h.project=t.project and t.status='y' and t.ano=h.ano
left join
	transfer as t2
on
	h.project=t.project and t.status='n' and t.ano=h.ano
where
	1=1".$where.$Twhere."
group by
	h.project
...
# 총 참여자
$sql = "select count(distinct(writer)) from transfer";
$man = $DB->FRow($sql);
# 오늘 작업
...
if($res['Count'] > 0)
{
	...
	<tr style="height:28px;text-align:center;">
		<td align="left" style="padding-left:10px;"><a href="pack'._LINK_.'.php?P='.$res[$i]['project'].'&Query='.$k_Query.'&Colm='.$k_Colm.'">'.RE(SLASH($res[$i]['project'],1), $k_Query,1).'</a></td>
