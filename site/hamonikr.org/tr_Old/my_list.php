...
elseif($k_Query!='' && $k_Colm=='') $where = " and (t.project like '%".$k_Query."%' or t.msgstr like '%".$k_Query."%')";
...
if($res['Count'] > 0)
{
	...
	<tr align="center" style="border-top:2px solid #666666;">
		...
		<td align="left" style="line-height:20px;">
			<div style="clear:both;"><ul style="border:0px solid;float:left;height:100%;">원 문 :&nbsp;</ul><ul class="bold F_00" style="float:left;width:80%;border:0px solid;word-break:break-all;">'.(($res[$i]['msgid']!='')?RE($res[$i]['msgid'],$k_Query,1):'&nbsp;').'</ul></div>
			<div style="clear:both;"><ul style="border:0px solid;float:left;height:100%;">번 역 :&nbsp;</ul><ul class="bold F_00" style="float:left;width:80%;border:0px solid;word-break:break-all;color:#ff6600;height:100%;">'.RE(SLASH($res[$i]['msgstr'],1),$k_Query,1).'</ul></div>
		</td>
		...
		<td>'.$res[$i]['regdate'].'</td>
		<td style="color:red;">'.(($res[$i]['status']=='y')?'○':'X').'</td>
		<td>'.number_format($res[$i]['vote']).'</td>
		<td style="line-height:23px;">';
if($res[$i]['status']=='n' && $res[$i]['vote']=='0') $HTML .= '<a href="javascript:;" class="btn btn-info btn-xs">수정</a><br /><a href="javascript:Del(\''.$res[$i]['t_idx'].'\');" class="btn btn-danger btn-xs">삭제</a>';
else $HTML .= '<a class="jqtooltip" title="내가 쓴 글 또는 추천이 있거나 최종 승인된 글입니다"><u>X</u></a>';
		$HTML .= '
		</td>
	</tr>
		';
